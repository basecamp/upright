...
`primary` marks the site that serves the app and status hostnames and runs the singleton jobs — the ones writing the shared `persistent` database, which must run in exactly one place. Gate them in `config/recurring.yml` with `Upright.current_site&.primary?`. Declaring two primaries raises on boot; declaring none is legal, and leaves the singletons ungated.

Every site exports `upright_primary_site` (1 on the primary, 0 elsewhere), `upright_persistent_db_up`, and `upright_rollup_last_run_timestamp_seconds`, which is what failover alerting needs. `Upright::HealthMetricsJob` refreshes them, so schedule it on every site — new installs get it from the generator, existing ones need it adding:
